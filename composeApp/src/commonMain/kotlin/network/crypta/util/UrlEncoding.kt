...
import network.crypta.util.URLEncoder.SAFE_URL_CHARACTERS
import network.crypta.util.URLEncoder.encode
...
/**
 * Provides functions for encoding and decoding URI components, similar to Freenet's
 * `URLEncoder` and `URLDecoder` utilities.
 *
 * The implementation avoids any dependency on JVM-only APIs, making it suitable for
...
object URLDecoder {
     ...
     */
    fun decode(s: String, tolerant: Boolean): String {
        if (s.isEmpty()) return ""
...
/**
 * Provides functions for percent-encoding strings for use in URIs.
 *
 * This encoder follows a specific set of rules for which characters are considered "safe"
 * and do not require encoding.
 */
object URLEncoder {
    /**
     ...
     *
     * This is a convenience overload for `encode(url, null, ascii)`.
     *
     ...
     * @see encode
     */
    fun encode(url: String, ascii: Boolean): String = encode(url, null, ascii)
     ...
     * @return The percent-encoded [String].
     */
    fun encode(url: String, force: String?, ascii: Boolean): String {

...
val appDisplayName = "Feed Reader & Publisher"
val appId = "feed-reader"
...
val launcherRelativePath = "bin/feed-reader.sh"
...
val signApp by
  tasks.registering(JavaExec::class) {
    ...
    inputs
      .dir(stageAppDir)
      .withPropertyName("feedReaderStagedBundleForSigning")
...
val verifyApp by
  tasks.registering(JavaExec::class) {
    ...
    inputs
      .dir(stageAppDir)
      .withPropertyName("feedReaderStagedBundleForVerification")
      .withPathSensitivity(PathSensitivity.RELATIVE)
    doFirst {
...
tasks.named<Test>("test") {
  ...
  inputs
    .dir(stageAppDir)
    .withPropertyName("feedReaderStagedBundle")
    .withPathSensitivity(PathSensitivity.RELATIVE)
  inputs.property("feedReaderAppVersion", project.version.toString())
  systemProperty("feedReader.stageDir", stageAppDir.get().asFile.absolutePath)
  systemProperty("feedReader.appVersion", project.version.toString())
}
